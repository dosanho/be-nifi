...
public class CriteriaParser {
    ...
    private static Pattern SpecCriteraRegex = Pattern.compile("^(\\w+?)(" + Joiner.on("|\\").join(SearchOperation.SIMPLE_OPERATION_SET) + ")(\\*?)(\\-?.+?)(\\*?)$");
    ...
    public Deque<?> parse(String searchParam) {
        ...
        for (String token : searchParam.split("\\s+")) {
            ...
            } else if (SearchOperation.LEFT_PARANTHESIS.equals(token)) {
                stack.push(SearchOperation.LEFT_PARANTHESIS);

            } else if (SearchOperation.RIGHT_PARANTHESIS.equals(token)) {
